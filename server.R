...
server <- function(input, output, session) {
  ...
  # Consolidated prcdata logic
  prcdata <- reactive({
    # Determine which page the user is on and load the corresponding dataset
    if (input$navbar == "Upload") {
    ...
    } else if (input$navbar == "Main App") {
      # User is on the 'Main App' (Use Pre-existing Dataset) page
      ...
      # User is on the 'Introduction' page, return NULL
      return(NULL)
    }
  ...
  # Observe and update gene selection based on available data
  observe({
    df <- prcdata()
    if (!is.null(df)) {
  ...
  # The rest of the server logic for plotting and downloading can be mirrored for both pages
  # (you can repeat the logic for 'Upload' and 'Existing' similarly)
  
  # The rest of the server logic follows here...
